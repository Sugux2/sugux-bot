...
class MyBot(BaseBot):
    ...
    async def on_start(self, session_metadata):
        print("✅ Бот Sugux в сети!")

    async def on_chat(self, user: User, message: str):
        # Если написали число — бот делает анимку под этим номером
        if message.isdigit():
            idx = int(message)
...
if __name__ == "__main__":
    keep_alive()
    # Авто-подстановка данных для запуска
    sys.argv = ["python", "main:MyBot", ROOM_ID, API_TOKEN]
    main()
